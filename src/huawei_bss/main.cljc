(ns huawei_bss.main
  "Kotodama WASM entrypoint for the Huawei_bss clean-room actor (L4) — Clojure port.
  ...
  facts (`huawei_bss.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "huawei_bss")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"huawei_bss.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "huawei_bss-compat" :tier tier :entities entities} 200])
...
(defn start! [] :huawei_bss-compat/ready)
